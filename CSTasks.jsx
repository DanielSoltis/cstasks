...
var CSTasks = (function(){
    ...
    ************/
	 ...
	 * @return {array} colors - A 2-dimensional array with corresponding RGBColor and CMYKColor objects in the form [[RGBColor,CMYKColor],[RGBColor2,CMYKColor2],...] of length n
	 */ 
    ...
    **************/
	 ...
	 * @param {DocumentColorSpace} colorspace - The desired colorspace: DocumentColorSpace.RGB or DocumentColorSpace.CMYK
	 * @return {boolean} match - True if the colors are the same, false if not
	 */     
    tasks.colorMatch = function(color1, color2, colorspace){
	 ...
	 * @param {DocumentColorSpace} colorspace - The desired colorspace: DocumentColorSpace.RGB or DocumentColorSpace.CMYK
	 * @return {number} index - Index of the matching color or -1 if no match
	 */  
    tasks.colorMatchToPalette = function(color, paletteArray, colorspace){
    	if (colorspace == DocumentColorSpace.RGB){
	 ...
	 * @param {array} paletteArray - Color palette as created from {@link initializeColorPalette}
	 * @param {DocumentColorSpace} colorspace - The desired colorspace: DocumentColorSpace.RGB or DocumentColorSpace.CMYK
	 * @return {array} paletteIndex - Array in which each element is the index of a pathItem's matching color (or -1 if no match)
	 */ 
    tasks.colorMatchItemsToPalette = function(pathItems, paletteArray, colorspace){
        var colorIndex = new Array(pathItems.length);
        for (var i = 0; i < pathItems.length; i++ ) {
            var itemColor = pathItems[i].fillColor;
            colorIndex[i] = tasks.colorMatchToPalette(itemColor, paletteArray, colorspace);
        }
        return colorIndex;
    };
    ...
    ****************/
	 ...
	 */  
    tasks.convertAllToColor = function(pathItems, endColor, opcty){
         for (var i = 0; i < pathItems.length; i++ ) {
	 ...
	 * @param {DocumentColorSpace} colorspace - The desired colorspace: DocumentColorSpace.RGB or DocumentColorSpace.CMYK
	 */ 
    tasks.convertMatchedItemsToColor = function(pathItems, startColor, endColor, colorspace){
        for (var i = 0; i < pathItems.length; i++ ) {
            if (tasks.colorMatch(pathItems[i].fillColor, startColor, colorspace) && !pathItems[i].layer.locked && !pathItems[i].locked) 
            	pathItems[i].fillColor = endColor;
        }
    };
	 ...
	 * @param {DocumentColorSpace} colorspace - The desired colorspace: DocumentColorSpace.RGB or DocumentColorSpace.CMYK
	 */ 
    tasks.convertToPalette = function(doc, pathItems, paletteArray, paletteIndex, colorspace){
        var unmatchedColors = [];
        var s = 0;
        ...
        if (unmatchedColors.length > 0){  
            ...
            unmatchedColors = tasks.getUniqueElements(unmatchedColors);
	 ...
	 */ 
    tasks.getUniqueElements = function(a) {
        if (a.length > 0){
            sorted =  a.sort();
            uniq = [sorted[0]];
